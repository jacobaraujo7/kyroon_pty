...
[![ci](https://github.com/cesarmod2017/kyroon_pty/actions/workflows/ci.yml/badge.svg)](https://github.com/cesarmod2017/kyroon_pty/actions/workflows/ci.yml)
[![pub points](https://badges.bar/kyroon_pty/pub%20points)](https://pub.dev/packages/kyroon_pty)
...
`kyroon_pty` always sets `TERM=xterm-256color` and `LANG=en_US.UTF-8` (so tools
...
* **Local** — the shell runs on *this* machine; bytes come from `kyroon_pty`.
...
```
   ...
   │  real PTY  (kyroon_pty / node-pty / …)     │        │  pub/sub + buffer     │        │  Flutter app   │
   │                                             │        │  (e.g. Redis)         │        │  (xterm)       │
...
1. **Own the real PTY on the host.** Spawn the shell/process in a pseudo-terminal
   *on the target machine* (this is where `kyroon_pty` itself can run, or
...
* `example/lib/pty_websocket_server.dart` — `PtyWebSocketServer`: runs on the
  **host** (a desktop app using kyroon_pty), spawns a real `Pty`, and serves it
...
```dart
import 'package:kyroon_pty_example/pty_session.dart';
...
* `lib/` — the Dart API in `kyroon_pty.dart`, calling the native library via
  `dart:ffi`. Bindings in `lib/src/kyroon_pty_bindings_generated.dart` are
  generated from `src/kyroon_pty.h` by [`package:ffigen`](https://pub.dev/packages/ffigen)
...
After editing the native header `src/kyroon_pty.h`, regenerate the FFI
bindings:
